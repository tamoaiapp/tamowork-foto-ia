// stress-setup-m5.mjs — Provisiona conta test-stress-m5 como PRO no Supabase
...
async function main() {
  console.log('=== Setup conta PRO test-stress-m5 ===\n')

  // 1. Criar usuario via Admin API
  console.log('1. Criando usuario via Admin API...')
  ...
  } else if (createResp.status === 422 || (typeof createResp.body?.message === 'string' && createResp.body.message.includes('already'))) {
  ...
  import('fs').then(({ default: fs }) => {
    fs.writeFileSync(
      'C:/Users/Notebook/tamowork-foto-ia/stress-m5-credentials.json',
      JSON.stringify({ userId, email: EMAIL, password: PASSWORD }, null, 2)
    )
    console.log('Credentials salvas em stress-m5-credentials.json')
  })
