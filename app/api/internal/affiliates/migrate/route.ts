...
export async function GET(req: NextRequest) {
  ...
  for (let index = 0; index < MIGRATIONS.length; index += 1) {
    ...
    try {
      const { error } = await (supabaseAdmin.rpc as unknown as (fn: string, args: { sql: string }) => Promise<{ error: { message: string } | null }>)("exec_sql", { sql });
      if (error) {
        steps.push({ index, ok: false, error: error.message });
      ...
      steps.push({
        index,
        ok: false,
        error: err instanceof Error ? err.message : "Erro ao executar migra̤̣o",
      });
      return NextResponse.json({ ok: false, steps }, { status: 500 });
    }
  }
